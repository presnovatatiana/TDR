...
if (navList) {
  navList.addEventListener('click', event => {
    if (!(event.target instanceof Element) || !event.target.matches('a')) return;
...
const accordionItems = document.querySelectorAll('.spot-accordion-item[data-spot]');
...
let activeSpotId = spotLayout ? spotLayout.dataset.activeSpot || DEFAULT_SPOT_ID : DEFAULT_SPOT_ID;
...
function focusMapOnSpot(id, zoomOverride) {
  ...
  const scale = clamp(zoomOverride || spot.zoom || 1.85, MIN_MAP_SCALE, MAX_MAP_SCALE);
  const position = getSpotStagePosition(spot, rect);

  const focusX = rect.width * (isDesktopMap() && !isPanelCollapsed() ? 0.36 : 0.5);
...
function setPanelCollapsed(collapsed, shouldFocus = true) {
  ...
  spotPanelToggle.setAttribute(
    'aria-label',
    collapsed ? 'Tauchspot-Menü ausklappen' : 'Tauchspot-Menü nach oben einklappen'
  );
...
if (mapStage && mapInner) {
  ...
  mapStage.addEventListener('wheel', event => {
    ...
    const nextScale = clamp(mapTransform.scale * zoomFactor, MIN_MAP_SCALE, MAX_MAP_SCALE);
...
const courses = {
  'reactivate': {
    ...
    description: 'Dein letzter Tauchgang ist schon Jahre her? Dann steige am besten mit unserem ReActivate-Programm wieder in den Sport ein. Gemeinsam frischen wir deine Kenntnisse auf, damit du erneut eigenständig tauchen kannst.',
    facts: 'Preis: 400 Euro<br />Dauer: 1 Tag, 7.00 bis 15.30 Uhr<br />Termine: Jeden Mittwoch',
    dates: [
      { value: 'Mittwoch, 07.00 bis 15.30 Uhr', label: 'Mittwoch, 07.00 bis 15.30 Uhr' }
    ]
  },
  'schnuppertauchen': {
    ...
    description: 'Noch nicht ganz sicher, ob Tauchen das Richtige für dich ist? Dann ist unser Schnuppertauchgang eine gute Wahl. Ein erfahrener Guide gibt dir einen Crashkurs, nimmt dich mit unter Wasser und zeigt dir, worauf es beim Tauchen ankommt.',
    facts: 'Preis: 150 Euro<br />Dauer: 3 Std., 13.00 bis 16.00 Uhr<br />Termine: Jeden Montag',
    dates: [
      { value: 'Montag, 13.00 bis 16.00 Uhr', label: 'Montag, 13.00 bis 16.00 Uhr' }
    ]
  },
  'open-water-diver': {
    ...
    description: 'Werde bei uns Open Water Diver! Die weltweit anerkannte Grundausbildung im Gerätetauchen ist der perfekte Einstieg in diesen Sport. Innerhalb von vier Tagen bringen wir dir alles bei, was du können musst, um eigenständig tauchen zu können.',
    facts: 'Preis: 500 Euro<br />Dauer: 2 Tage, jew. 7.00 bis 15.30 Uhr<br />Termine: Immer dienstags bis freitags',
    dates: [
      { value: 'Dienstag & Mittwoch, 07.00 bis 15.30 Uhr', label: 'Dienstag & Mittwoch, 07.00 bis 15.30 Uhr' },
      { value: 'Donnerstag & Freitag, 07.00 bis 15.30 Uhr', label: 'Donnerstag & Freitag, 07.00 bis 15.30 Uhr' }
    ]
  },
  'advanced-open-water-diver': {
    ...
    description: 'Erweitere Deine Fähigkeiten im Advanced Open Water Diver! Gemeinsam absolvieren wir fünf Abenteuertauchgänge, inklusive Tieftauchen sowie Unterwassernavigation. Nach diesem Kurs darfst du bis zu 30 Meter tief tauchen.',
    facts: 'Preis: 400 Euro<br />Dauer: 2 Tage, jew. 7.00 bis 15.30 Uhr<br />Termine: Immer miittwochs bis samstags',
    dates: [
      { value: 'Mittwoch & Donnerstag, 07.00 bis 15.30 Uhr', label: 'Mittwoch & Donnerstag, 07.00 bis 15.30 Uhr' },
      { value: 'Freitag & Samstag, 07.00 bis 15.30 Uhr', label: 'Freitag & Samstag, 07.00 bis 15.30 Uhr' }
    ]
  },
  'rescue-diver': {
    title: 'Rescue Diver',
...
const bookingDescription = document.querySelector('#booking-course-description');
...
function validateBookingForm(showErrors = true) {
  ...
  const fields = Array.from(bookingForm.querySelectorAll('input, select, textarea')).filter(field => field.type !== 'hidden' && field.name !== '_honey');
...
if (bookingForm) {
  ...
  setCourse(courseSelect ? courseSelect.value : 'reactivate');
}

document.querySelectorAll('.book-course-link[data-course]').forEach(link => {
  link.addEventListener('click', () => {
    setCourse(link.dataset.course || 'reactivate');
  });
});
